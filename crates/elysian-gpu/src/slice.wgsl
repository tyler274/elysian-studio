...
@group(0) @binding(5) var<storage, read_write> segments: array<LayerSeg>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let t = gid.x;
    let layer = gid.y;
    if t >= params.tri_count || layer >= params.layer_count {
        return;
    }
    let tri = triangles[t];
    ...
    if slot < arrayLength(&segments) {
        segments[slot] = LayerSeg(p0.xy, p1.xy, layer, 0u, 0u, 0u);
    }
}
