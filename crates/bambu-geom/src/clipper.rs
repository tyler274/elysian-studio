...
use clipper2_rust::clipper::{inflate_paths_64, union_subjects_64};
...
pub fn union_polygons(polygons: &[Polygon]) -> Vec<Polygon> {
    ...
    let subjects: Paths64 = polygons.iter().map(|p| to_path64(p)).collect();
    union_subjects_64(&subjects, FillRule::NonZero)
        .iter()
        .map(from_path64)
        .filter(|p| p.len() >= 3)
        .collect()
