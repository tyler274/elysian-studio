...
impl Compositor {
    ...
    pub async fn request<W: compositor::Window>(
        ...
        let limits = limits.into_iter().map(|limits| {
            crate::gpu::device_limits(&adapter, limits)
        });

        let mut errors = Vec::new();

        for required_limits in limits {
            let features = crate::gpu::ray_query_features(&adapter);
            let experimental =
                crate::gpu::experimental_features(&adapter);
            let result = adapter
            ...
            match result {
                ...
                Err(error) => {
                    errors.push((required_limits.clone(), error));
                }
            }
            ...
            // Ray-query device creation can fail; keep iced running without RT.
